...
# Create header frame
header_frame = tk.Frame(window, bg="#76b5c5")
...
pitch_label.grid(row=0, column=1, padx=10, pady=5)
pitch_entry.grid(row=1, column=1)
...
yaw_label.grid(row=0, column=2, padx=10, pady=5)
yaw_entry.grid(row=1, column=2)
...
throttle_label.grid(row=5, column=0, padx=30, pady=5, sticky="w")
throttle_slider.grid(row=6, column=0, columnspan=5, padx=30, pady=0, sticky="w")

# Create label and entry box for PID Roll
pid_roll_label = tk.Label(data_frame, text="PID ROLL", font=("Arial", 10, "bold"))
...
pid_roll_label.grid(row=3, column=8, padx=50, pady=5, sticky="w")
pid_roll_entry.grid(row=4, column=8, padx=20, pady=5)

# Create label and entry box for PID Pitch
pid_pitch_label = tk.Label(data_frame, text="PID PITCH", font=("Arial", 10, "bold"))
...
pid_pitch_label.grid(row=5, column=8, padx=50, pady=5, sticky="w")
pid_pitch_entry.grid(row=6, column=8, padx=20, pady=5)
...
com_port_label.grid(row=11, column=0, padx=35, pady=5, sticky="w")
com_port_dropdown.grid(row=12, column=0, padx=10, pady=5)
baud_rate_label.grid(row=11, column=1, padx=20, pady=5, sticky="w")
baud_rate_dropdown.grid(row=12, column=1, padx=10, pady=5)

# Adjust grid configuration for remaining elements
pid_roll_label.grid(row=4, column=8, padx=50, pady=5, sticky="w")
pid_roll_entry.grid(row=5, column=8, padx=20, pady=5)
pid_pitch_label.grid(row=6, column=8, padx=50, pady=5, sticky="w")
pid_pitch_entry.grid(row=7, column=8, padx=20, pady=5)

# Create footer frame
footer_frame = tk.Frame(window, bg="#323232", borderwidth=1, relief="groove")
footer_frame.pack(fill=tk.X, side="bottom")

# Create a container frame for both text labels
footer_text_frame = tk.Frame(footer_frame, bg="#323232")
...
student_info_label = tk.Label(
    footer_text_frame,
    text="4121600043  |  Fatkhur Rohman Fajar Pramuji\n4121600048  |  Reza Zakaria",
    font=("Arial", 7),
    bg="#323232",
    justify="left",
)
student_info_label.pack(anchor="w", padx=10, pady=10)
student_info_label.configure(bg="#323232", fg="white")
...
mekatronika_label.configure(bg="#323232", fg="white")
...
connect_button.grid(row=11, column=2, padx=30, pady=5)
disconnect_button.grid(row=12, column=2, padx=30, pady=5)

# Update data every 100 milliseconds
window.after(10, read_serial_data)
